...
# cd /nfsd/gracedata/dosso/creditToRDF/target/slurm/run_whole_db_virtuoso
...
VALPROP=${JARDIR}/properties/values_alpha.properties  # properties file with the parameters of the execution
SLURMDIR=${JARDIR}/slurm/run_queries_whole_db_virtuoso # directory where these files are located 
...
JBNM=wikiWhole								# name of the job
FOUT="${OUTPUTDIR}/wikiWhole.txt" 			# file di standard output
FERR="${OUTPUTDIR}/wikiWhole_err.txt" 		# file di standard error


# send request to slurm
sbatch \
	--mail-user=$MAIL \
	--mail-type=$MTYP \
	--ntasks=$NTSK \
	...
	--error=$FERR <<EOT 
#!/bin/bash
srun ${SLURMDIR}/wun_whole_db_virtuoso.sh ${JARDIR} ${MASTERDIR} ${VALPROP};
EOT