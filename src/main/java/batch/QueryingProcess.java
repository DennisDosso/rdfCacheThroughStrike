...
public class QueryingProcess {
     ...
     * */
    protected void writeDownResultsAboutLineage(ReturnBox constructBox) {
        try {
            this.constructFw.write(constructBox.queryTime + "\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
        ...
        try(BufferedReader reader = Files.newBufferedReader(Paths.get(ProjectPaths.supportTextFile))) {
            ...
            while( (line = reader.readLine()) != null ) { // read each construct query
                ...
                if(lineage.size() > 0) {
                    ...
                    this.writeDownResultsAboutLineage(constructBox);
